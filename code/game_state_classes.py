...
class Game_state:
    ...
    def __init__(self):
        ...
        # Storing the chessboard image from previous iteration
    ...
    def register_move(self, move, board_image):
        if move in self.board.legal_moves:
            ...
            self.executed_moves = np.append(
                self.executed_moves, self.board.san(move))
            self.board.push(move)
    ...
    def get_square_center(self, square_name):
        row, column = convert_square_name_to_row_column(
            square_name, self.we_play_white)
        position = self.board_position_on_screen
        centerX = int(position.minX + (column + 0.5) *
    ...
    def play_next_move(self):
        ...
        pyautogui.dragTo(centerXOrigin, centerYOrigin +
                         1, button='left', duration=0.01)
        ...
        if best_move.promotion != None:
            ...
            pyautogui.dragTo(centerXDest, centerYDest + 1,
